...
if(savedata){
  write.csv(df, paste0("saveddata/",lubridate::today(),".csv"))
}
```
...
if("ema14" %in% colnames(df)){
  dftmp <- df %>% select(Symbol, `5 Minutes`:Monthly, ema14, ma55, ma200, Sup3, Sup2, Sup1, Pivot, Res1, Res2, Res3, Last, `Chg. %`)
  ...
  dftmp <- dftmp %>% mutate(DistEma14 = round(100*(Last - ema14)/Last,2),
    ...
    mutate(across(ema14:ma200, ~ round(.x,2)))
  ...
  formatStyle(columns = c("DistEma14", "DistMa55", "DistMa200"), backgroundColor = styleInterval(c(0), c("tomato", "palegreen")))
